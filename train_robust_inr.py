...
def config_parser():
    ...
    parser.add_argument('--grad_epsilon', default=0.9, type=float, help='constrain the grad of image pixel when attack on INR')
...
def fgsm_attack_img(model, criterion, image, label, norm, epsilon):
    model.eval()
    # 将图像和标签移动到设备上
...
def pgd_attack_linf_img(model, criterion, images, labels, norm, epsilon=8 / 255, alpha=1.6 / 255, num_iter=10):
    r"""
    Overridden.
    """
    ...
    output = model(norm(adv_images))
    _, predicted = output.max(1)
    return adv_images, predicted
...
# attack parameters
    attack_net_FGSM_epsilon = args.attack_net_FGSM_epsilon
    # attack_noise_std = args.attack_noise_std

    attack_net_PGD_epsilon = args.attack_net_PGD_epsilon
    ...
    attack_net_DSPGD_num_iter = args.attack_net_DSPGD_num_iter
    attack_net_DSPGD_net_epsilon = args.attack_net_DSPGD_net_epsilon
    attack_net_DSPGD_net_alpha = 2.5 * attack_net_DSPGD_net_epsilon / attack_net_DSPGD_num_iter
    attack_net_DSPGD_image_epsilon = args.attack_net_DSPGD_image_epsilon
    attack_net_DSPGD_image_alpha = args.attack_net_DSPGD_image_alpha
    
    attack_net_CW_epsilon = args.attack_net_CW_epsilon
    attack_net_CW_num_iter = args.attack_net_CW_num_iter
    attack_net_CW_alpha = 2.5 * attack_net_CW_epsilon / attack_net_CW_num_iter

    train_net_PGD_epsilon = args.train_net_PGD_epsilon
...
# choose the training type
    if args.method == "normal":
        ...
        method = "robust_inr_epsilon_" + str(train_net_PGD_epsilon) + "_alpha_" + str(
            train_net_PGD_alpha) + "_num_iter_" + str(train_net_PGD_num_iter) + "_lambda_w_" + str(
            args.lambda_w) + "_lambda_i_" + str(args.lambda_i)
    else:
...
# if run part of the whole test dataset
        ...
        log_stats["total_num"] = log_stats["total_num"] + 1
# generate rand message
        ...
        if args.method == "normal":
            ...
            inr_net, embed_fn = adversarial_train(input_img, msg, msg_decoder, loss_w, loss_i, data_aug, vqgan_to_imnet, train_net_PGD_epsilon, train_net_PGD_alpha,
                                                  train_net_PGD_num_iter, inr_weights_dir, image_idx, args)
        ...
        if args.attack_clean:
            ...
            img_clean = inference_inr(inr_net, embed_fn, args)
            for name, attack in attacks.items():
                imgs_aug = attack(img_clean.permute(2, 0, 1).unsqueeze(0))
                ...
                correct_bit = torch.sum(diff, dim=-1)
                bit_acc = correct_bit / diff.shape[-1] # b k -> b
                log_stats.setdefault(f'correct_bit_{name}', []).append(correct_bit.item())
        ...
        if args.attack_net_FGSM:
            ...
            inr_net_attack = attack_inr_FGSM(msg_decoder, loss_w, loss_i, inr_net, embed_fn,
                                                                        msg, target_msg, vqgan_to_imnet, attack_net_FGSM_epsilon, args)
            img_attack = inference_inr(inr_net_attack, embed_fn, args)
            decoded = msg_decoder(vqgan_to_imnet(img_attack.permute(2, 0, 1).unsqueeze(0)))
            ...
            img = torch.clamp(img_attack, 0, 1)
            ref = inputs.permute(0, 2, 3, 1).squeeze(0)
            img_loss = img2mse(img, ref)
            psnr = mse2psnr(img_loss)
            lpips_val = compute_lpips(img, ref)
        ...
        if args.attack_net_PGD:
            name = 'attack_net_PGD'
            inr_net_attack = attack_inr_PGD_linf(msg_decoder, loss_w, loss_i, inr_net, embed_fn,
        ...
        if args.attack_net_DSPGD:
            for DSPGD_image_epsilon in attack_net_DSPGD_image_epsilon:
                name = f'attack_net_DSPGD_{DSPGD_image_epsilon}'
                inr_net_attack = attack_inr_DSPGD_linf(msg_decoder, loss_w, loss_i, inr_net, embed_fn,
                                                                msg, target_msg, vqgan_to_imnet, attack_net_DSPGD_net_epsilon,
                                                                attack_net_DSPGD_net_alpha, attack_net_DSPGD_num_iter,
                                                                DSPGD_image_epsilon/255.0, attack_net_DSPGD_image_alpha, args)
                img_attack = inference_inr(inr_net_attack, embed_fn, args)
                decoded = msg_decoder(vqgan_to_imnet(img_attack.permute(2, 0, 1).unsqueeze(0)))
                # loss = criterion(outputs, targets)
                # test_loss += loss.item()
...
# DSPGD_CW
        if args.attack_net_DSPGD:
            for DSPGD_image_epsilon in attack_net_DSPGD_image_epsilon:
                name = f'attack_net_DSPGD_CW_{DSPGD_image_epsilon}'
                inr_net_attack = attack_inr_DSPGD_linf(msg_decoder, CW_msg_loss, loss_i, inr_net, embed_fn,
                                                                msg, target_msg, vqgan_to_imnet, attack_net_DSPGD_net_epsilon,
                                                                attack_net_DSPGD_net_alpha, attack_net_DSPGD_num_iter,
                                                                DSPGD_image_epsilon/255.0, attack_net_DSPGD_image_alpha, args)
                img_attack = inference_inr(inr_net_attack, embed_fn, args)
        ...
        if args.attack_net_CW:
            name = 'attack_net_CW'
            inr_net_attack = attack_inr_PGD_linf(msg_decoder, CW_msg_loss, loss_i, inr_net, embed_fn,
        ...
        results = {}
        results["total_num"] = log_stats["total_num"]
        for name, attack in attacks.items():
            result = {
            ...
            for i in range(0, nbit+1):
                result[f'{i}'] = log_stats[f'correct_bit_{name}'].count(i) / log_stats["total_num"]
        ...
        if args.attack_net_DSPGD:
            for DSPGD_image_epsilon in attack_net_DSPGD_image_epsilon:
                name = f'attack_net_DSPGD_{DSPGD_image_epsilon}'
                result = {
                    "net_epsilon": attack_net_DSPGD_net_epsilon,
                    "net_alpha": attack_net_DSPGD_net_alpha,
                    "image_epsilon": DSPGD_image_epsilon,
                    "image_alpha": attack_net_DSPGD_image_alpha,
                    "num_iter": attack_net_DSPGD_num_iter,
                    "accuracy": log_stats[f'average_acc_{name}'],
                    "psnr": log_stats[f'psnr_val_mean_{name}'],
                    "ssim": log_stats[f'ssim_val_mean_{name}'],
                    "lpips": log_stats[f'lpips_val_mean_{name}']
