...
from jpeg import Jpeg, JpegSS, JpegMask
...
def JPEGMASK(x, quality):
    jpegmask = JpegMask(quality)
    x = (jpegmask(x)+1.0)/2.0
    return x

def JPEGSS(x, quality):
    jpegss = JpegSS(quality)
    x = (jpegss(x)+1.0)/2.0
    return x

def REALJPEG(x, quality):
    jpeg = Jpeg(quality)
    x = (jpeg(x)+1.0)/2.0
    return x
...
class ImageAug(nn.Module):
    ...
    def forward(self, x):
        index = np.random.choice(len(self.attacks), 1, p=self.p)[0]
        random_noise_layer = self.attacks[index]
        return random_noise_layer(x)
